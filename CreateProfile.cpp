...
std::string promptUserForMinecraftPath2() { 
     const char* folderPath = tinyfd_selectFolderDialog(
            "Please select your Minecraft folder",
            NULL 
        );
        if (folderPath) {
            std::string inputPath(folderPath);
...
std::string promptUserForMinecraftPath() {
    while (true) {
        const char* folderPath = tinyfd_selectFolderDialog(
            "Could not find Minecraft folder automatically.\nPlease select your Minecraft folder",
            NULL 
        );
        ...
        if (!folderPath) {
            tinyfd_messageBox(
                    ...
                    "The path you selected does not exist. Please try again.",
                    "ok",
                    "error",
                    1
                );
...
void writejsonSomethin(int mem) {
    ...
    std::string javaArgs =
    "-Xmx" + std::to_string(mem) + "M " // max
    "-Xms" + std::to_string(mem) + "M " // min
